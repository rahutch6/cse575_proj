...
roses = list(pathlib.Path(sparse_dir).glob('roses/*'))
print(f'\tnew roses: {len(roses)}')
...
for i in range(len(sunflowers)):
    ...
    if i%2==0:
        shutil.copyfile(inPath, sparse_dir + outPath)
        shutil.copyfile(inPath, gen_dir + outPath)
sunflowers = list(pathlib.Path(sparse_dir).glob('sunflowers/*'))
print(f'\tnew sunflowers: {len(sunflowers)}')
